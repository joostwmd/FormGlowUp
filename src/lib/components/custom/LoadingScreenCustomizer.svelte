<script lang="ts">
	...
	import { formStructureStore } from '$lib/form/stores';

	function selectLoader(loaderValue: string) {
		formStructureStore.update((prev) => ({ ...prev, loader: loaderValue }));
	}
</script>
	...
	<div class="grid grid-cols-2 gap-4">
		{#each LOADERS as loader}
			<button on:click={() => selectLoader(loader.value)} class="">
				<Card.Root
					class={cn($formStructureStore.loader === loader.value && 'border-2 border-primary')}
				>
					<Card.Content>
