<script lang="ts">
	...
	export let publicFormUrl: string;

	function openGmail() {
		const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&body=${encodeURIComponent(publicFormUrl)}`;
		console.log(gmailUrl);
		window.open(gmailUrl, '_blank');
	}
</script>

<Button on:click={openGmail}>
